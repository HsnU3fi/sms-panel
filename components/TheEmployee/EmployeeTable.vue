...
//methods
const getEmployeeApi = async (page) => {
  const result = await employeesStore.getEmployees(page)
...
const deleteItemHandler = async () => {
  const result = await employeesStore.deleteItem(stagedItemToDelete.value.id)
  if (result.status === 200) {
    getEmployeeApi()
    okAlert()
  } else {
